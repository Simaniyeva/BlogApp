...
public class BlogManager : IBlogService
{
    ...
    public async Task<IDataResult<List<BlogGetDto>>> GetAllAsync(bool getDeleted, params string[] includes)
    {
        ...
        return new SuccessDataResult<List<BlogGetDto>>(_mapper.Map<List<BlogGetDto>>(blogs.OrderByDescending(p => p.CreatedDate).ToList()));
    }
    ...
    public async Task<IResult> CreateAsync(BlogPostDto dto)
    {
        ...
        FillBlog(blog, dto.formFiles, dto.TagIds);
    ...
    {
        Blog blog = await _unitOfWork.BlogRepository.GetAsync(b => b.Id == dto.Id && !b.isDeleted, "BlogImages", "BlogTags.Tag", "Reviews");
        if (blog.BlogTags.ToList().Count != 0)
        {
            foreach (BlogTag tag in blog.BlogTags.ToList())
            {
                _unitOfWork.BlogTagRepository.Delete(tag);
                await _unitOfWork.SaveAsync();
                blog.BlogTags.Remove(tag);
            }
        }
        if (blog.BlogImages.ToList().Count != 0)
        {
            foreach (BlogImage image in blog.BlogImages.ToList())
            {
                File.Delete(Path.Combine(_env.WebRootPath, "uploads/blog", image.ImagePath));
                _unitOfWork.BlogImageRepository.Delete(image);
                await _unitOfWork.SaveAsync();
    ...
    private async void FillBlog(Blog blog, List<IFormFile> files,List<int>TagIds)
    {
        if (files is not null)
        {
            AddBlogImages(blog, files);
        }
        if (TagIds.Count > 0)
        {
            AddTags(blog, TagIds);
    ...
    private async void AddBlogImages(Blog blog, List<IFormFile> files)
    {
        foreach (IFormFile file in files)
        {
            ...
            {
                Blog = blog,
                ImagePath = file.FileCreate(_env.WebRootPath, "uploads/blog")
            };
            blog.BlogImages.Add(image);
        }
    }

    private async void AddTags(Blog blog, List<int> TagIds)
    {
